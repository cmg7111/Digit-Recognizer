       ...
       <article class="container">
        <div id="text_div">
         <form method="post" action="" id="form1">
          <input type="text" name="img_url" placeholder="Enter Image URL or Drag and Drop Image">
...
if(isset($_POST['get_image']))
{
 ...
 echo ("<script language=javascript>showfiles_url($flag_url);</script>");
        ...
        xhr.onreadystatechange = function() {
          if (xhr.readyState  == xhr.DONE)     {
            if (xhr.status === 200) { 
                //var list = JSON.parse(xhr.response);
                var image = new Image();
                image.src = './image.png'+'?_='+ new Date().getTime();
                image.id="uploaded"
                image.width=540;
                
                var imgEle = document.getElementById("uploaded");
                res=xhr.response;
                console.log("작업완료_drag and drop");
                if(imgEle){
                    holder.removeChild(imgEle);
                    holder.appendChild(image);
                }
